...
void GCHD::configureHDMI()
{
	...
	readDevice0x9DCD(0x3e); //EXPECTED 0x3e
	readDevice0x9DCD(0x0b); //EXPECTED 0xb2
	readDevice0x9DCD(0x3f); //EXPECTED 0xb3
	mailWrite( 0x4e, VC{0x00, 0xce} );
	readDevice0x9DCD(0x34); //EXPECTED 0xc8
	readDevice0x9DCD(0x33); //EXPECTED 0xb3
	readDevice0x9DCD(0x31); //EXPECTED 0x02
	readDevice0x9DCD(0x31); //EXPECTED 0xb2
	mailWrite( 0x4e, VC{0x1b, 0x30} );
	readDevice0x9DCD(0x2b); //EXPECTED 0xb2
	readDevice0x9DCD(0x77); //EXPECTED 0x6c
	readDevice0x9DCD(0x74); //EXPECTED 0xa2
	readDevice0x9DCD(0x76); //EXPECTED 0x1a
	readDevice0x9DCD(0x20); //EXPECTED 0xb2
	...
	readDevice0x9DCD(0x77); //EXPECTED 0x6c
	readDevice0x9DCD(0x76); //EXPECTED 0x1a
	readDevice0x9DCD(0x75); //EXPECTED 0x32
	readDevice0x9DCD(0x77); //EXPECTED 0x6c
	readDevice0x9DCD(0x3f); //EXPECTED 0xb0
	mailWrite( 0x4e, VC{0x00, 0xcc} );

	if( settings_->getColorSpace()==ColorSpace::RGB ) {
	...
	if( settings_->getColorSpace()==ColorSpace::RGB ) {
		mailWrite( 0x4e, VC{0x9b, 0xc1} );
		mailWrite( 0x4e, VC{0x9c, 0x0c} );
	}
	...
	if( settings_->getColorSpace()==ColorSpace::RGB ) {
		mailWrite( 0x4e, VC{0xa9, 0xcc} );
	} else {
		mailWrite( 0x4e, VC{0xa9, 0xec} );
	}
	mailWrite( 0x4e, VC{0xaa, 0xcc} );
	readDevice0x9DCD(0x8d); //EXPECTED 0xba
	readDevice0x9DCD(0x8a); //EXPECTED 0xae
	mailWrite( 0x4e, VC{0xb2, 0xcc} );
	...
	//Separated into two blocks because I would like to remove conditional around 1st once I prove it is harmless.
	if ( deviceType_ == DeviceType::GameCaptureHD ) {
	...
	mailRead( 0x33, 1 ); //EXPECTED {0xee} //Theory read point
	mailWrite( 0x33, VC{0x99, 0x89, 0xf5} );
	...
	mailWrite( 0x33, VC{0x10, 0x01, setValue} );
#else
	mailWrite( 0x33, VC{0x10, 0x01, 0x71} );
#endif
	...
	if ( deviceType_ == DeviceType::GameCaptureHDNew )  {
		mailWrite( 0x4c, VC{0x73, 0x98, 0x7b, 0x7e} );
		mailWrite( 0x4c, VC{0x76, 0xc7, 0xf2, 0xef, 0xd8, 0x8b, 0xfb, 0x75, 0xa4, 0xe2, 0x9f, 0x06, 0x79, 0xa3, 0x56, 0xd2, 0x45, 0xa6, 0x3b} );
	}
	mailWrite( 0x33, VC{0x99, 0x89, 0x89} );
	mailRead( 0x33, 1 ); //EXPECTED {0x6e}

	if ( deviceType_ == DeviceType::GameCaptureHDNew ) {
		mailWrite( 0x4c, VC{0x72, 0x8b} );
		mailWrite( 0x4c, VC{0xc0, 0x88} );
	} else {
	...
	if (deviceType_ == DeviceType::GameCaptureHDNew ) {
		value &= ~0xC0; //not sure why this bit is clear on HDNew.
	}
	...
	if (deviceType_ == DeviceType::GameCaptureHDNew ) {
		readDevice0x9DCD(0x78); //EXPECTED 0x94
		readDevice0x9DCD(0x7f); //EXPECTED 0xac
		readDevice0x9DCD(0x7e); //EXPECTED 0xd4
		readDevice0x9DCD(0x7d); //EXPECTED 0xb1
		readDevice0x9DCD(0x7c); //EXPECTED 0xb2
		readDevice0x9DCD(0x7b); //EXPECTED 0xaa
		readDevice0x9DCD(0x7a); //EXPECTED 0xb2
	}
	readDevice0x9DCD(0x76); //EXPECTED 0x1a
	...
	if (deviceType_ == DeviceType::GameCaptureHD ) {
		...
		mailWrite( 0x4e, VC{0x00, 0xcc} );
		readDevice0x9DCD(0x6a); //EXPECTED 0xcd
		readDevice0x9DCD(0x64); //EXPECTED 0xb6
		readDevice0x9DCD(0x63); //EXPECTED 0xd7
		readDevice0x9DCD(0x3f); //EXPECTED 0xb2
		mailWrite( 0x4e, VC{0x00, 0xce} );
		readDevice0x9DCD(0x16); //EXPECTED 0xb5
		readDevice0x9DCD(0x17); //EXPECTED 0x32
		...
		readDevice0x9DCD(0x34); //EXPECTED 0xc8
		readDevice0x9DCD(0x33); //EXPECTED 0xb3
		readDevice0x9DCD(0x31); //EXPECTED 0xa2
		readDevice0x9DCD(0x31); //EXPECTED 0xb2
		mailWrite( 0x4e, VC{0x1b, 0x30} );
		readDevice0x9DCD(0x2b); //EXPECTED 0xb2
		readDevice0x9DCD(0x77); //EXPECTED 0x6c
	...
	if ( deviceType_ == DeviceType::GameCaptureHDNew )  {
		readDevice0x9DCD(0x77);
		readDevice0x9DCD(0x76);
		readDevice0x9DCD(0x75);
		readDevice0x9DCD(0x77);
